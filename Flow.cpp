...
void stream::input(ifstream& file) //input text from text file
{
	if (file.eof() != true)
	{
		...
		while (!file.eof())
		{
			...
			if (step == 1)
				throw out_of_range("wrong string");
			this->File.push_back(str);
		}
	}
}

void stream::SetVertexCount() 
{
...
void stream::CreateAdjacencyMatrix() 
{
	int source = -1;
	int sink = -1;
	...
	if (sink == -1 || source== -1)
		throw out_of_range("invalid graph");
	this->All_vertex.swap(0, source);
	this->All_vertex.swap(this->All_vertex.get_size() - 1, sink);
...
bool stream::IsTransport_network()
{
	...
	Find(0, this->All_vertex.get_size() - 1, temp_matrix, this_matrix);
	return this_matrix;
}
void stream::Find(int top, int end, int** color_matrix, bool& temp)
{
	int match = 0;
	for (int i = 0; i < this->All_vertex.get_size(); i++)
	{
...
int stream::getMaxFlow() //pushing the pre-stream algorithm
{
	if (this->IsTransport_network() != true)
